...
const SUPABASE_URL           = process.env.SUPABASE_URL  || 'https://cecggsnvrfnxlpfhoyip.supabase.co';
const SUPABASE_KEY           = process.env.SUPABASE_KEY  || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNlY2dnc252cmZueGxwZmhveWlwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NzgyMTA0NiwiZXhwIjoyMDkzMzk3MDQ2fQ.6nNP1DPVQgDRv2QEO_6LP96Ow5v6T0RZxpUBplrmPXY';
const ADMIN_EMAIL            = process.env.ADMIN_EMAIL    || 'wafuladuncan@yahoo.com';
const ADMIN_PASSWORD         = process.env.ADMIN_PASSWORD || '@Gunners2007';
...
// CORS — allow the dashboard/site origin
app.use((req, res, next) => {
...
function generateToken(email) {
  ...
  const signature = crypto.createHmac('sha256', ADMIN_API_KEY).update(payload).digest('hex');
...
function verifyToken(token) {
  ...
  const expectedSignature = crypto.createHmac('sha256', ADMIN_API_KEY).update(payload).digest('hex');
...
async function handleC2BConfirmation(req, res) {
  ...
  try {
    ...
    const transId    = payload.TransID || payload.transId || null;
    const transTime  = payload.TransTime || payload.transTime || null;
    const transAmount = parseFloat(payload.TransAmount || payload.transAmount || 0);
    const msisdn     = payload.MSISDN || payload.msisdn || null;
    ...
    const billRef    = payload.BillRefNumber || payload.billRefNumber || '';
